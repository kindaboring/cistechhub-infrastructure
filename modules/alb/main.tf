...
resource "aws_security_group" "alb" {
  ...
  # HTTP access from anywhere (redirect to HTTPS)
  ingress {
    from_port   = 80
    to_port     = 80
    protocol    = "tcp"
...
resource "aws_lb" "main" {
  name               = "${var.environment}-cistechhub-alb"
  internal           = false
...
resource "aws_wafv2_web_acl" "main" {
  ...
  name  = "${var.environment}-cistechhub-waf"
  scope = "REGIONAL"
  ...
  visibility_config {
    cloudwatch_metrics_enabled = true
    metric_name                = "${var.environment}-cistechhub-waf"
    sampled_requests_enabled   = true
  }
